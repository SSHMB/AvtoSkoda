...
function NavLink({ label, to }) {
  return (
    <Link to={to} className="flex items-center gap-1 text-sm font-medium text-gray-800 hover:text-red-600 transition-colors whitespace-nowrap">
      {label}
...
function CarCard({ car, onLikeToggle, initialLiked }) {
  ...
  const imgSrc = car.img || car.image;
  const title = car.name || `${car.brand || ""} ${car.model || ""}`.trim();
  const subtitle = car.kuzov || car.spec;
  const engine = car.dvigatel || car.fuel;
  const gearbox = car.korobka || car.transmission;

  return (
    <div className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col">
      {/* Image */}
      <div className="relative h-48 overflow-hidden bg-gray-100">
        ...
        <div className="absolute top-3 right-3 flex gap-1.5">
          <button
            onClick={handleLike}
            className={`w-8 h-8 rounded-full flex items-center justify-center backdrop-blur-sm border shadow transition-all
              ${liked ? "bg-red-500 border-red-500 text-white" : "bg-white/80 border-white/60 text-gray-600 hover:border-red-400"}`}
          >
          ...
          <button
            ...
            className={`w-8 h-8 rounded-full flex items-center justify-center backdrop-blur-sm border shadow transition-all
              ${compared ? "bg-blue-500 border-blue-500 text-white" : "bg-white/80 border-white/60 text-gray-600 hover:border-blue-400"}`}
          >
            <GitCompare size={13} />
          </button>
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 px-3 pb-2.5">
          ...
          {car.credit && (
            <p className="text-red-300 text-[11px] font-medium">
      ...
      <div className="flex flex-col flex-1 p-3.5">
        <div className="mb-2.5">
          <h3 className="font-black text-gray-900 text-sm leading-tight">{title}</h3>
          {subtitle && <p className="text-xs text-gray-400 mt-0.5">{subtitle}</p>}
        </div>

        <div className="grid grid-cols-2 gap-1.5 mb-3">
          {engine && (
            <div className="flex items-center gap-1.5 bg-gray-50 rounded-xl px-2.5 py-1.5 text-xs text-gray-600">
              <Zap size={10} className="text-red-500 flex-shrink-0" />
              {engine}
            </div>
          )}
          {gearbox && (
            <div className="flex items-center gap-1.5 bg-gray-50 rounded-xl px-2.5 py-1.5 text-xs text-gray-600">
              <Settings size={10} className="text-red-500 flex-shrink-0" />
              {gearbox}
            </div>
          )}
          {car.mileage != null && (
          ...
          {car.color && (
            <div className="flex items-center gap-1.5 bg-gray-50 rounded-xl px-2.5 py-1.5 text-xs text-gray-600">
              <CheckCircle size={10} className="text-red-500 flex-shrink-0" />
              {car.color}
            </div>
          )}
        </div>
        ...
        <div className="mt-auto flex gap-1.5">
          <button className="flex-1 bg-red-600 hover:bg-red-700 active:scale-95 text-white text-xs font-bold py-2 rounded-xl transition-all">
            Узнать цену
          </button>
          <button className="flex-1 bg-gray-900 hover:bg-gray-800 active:scale-95 text-white text-xs font-semibold py-2 rounded-xl transition-all">
            Подробнее
          </button>
        </div>
      </div>
